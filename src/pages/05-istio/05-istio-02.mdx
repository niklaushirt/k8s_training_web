...
1.  Execute the following command to get the latest ISTIO source:
    ...
    #Linux (Lab VM)
    wget https://github.com/istio/istio/releases/download/1.27.3/istio-1.27.3-linux-amd64.tar.gz
    tar xfvz istio-1.27.3-linux-amd64.tar.gz     
    
    #Mac
    wget https://github.com/istio/istio/releases/download/1.27.3/istio-1.27.3-osx.tar.gz
    tar xfvz istio-1.27.3-osx.tar.gz
    
    #Windows
    Download https://github.com/istio/istio/releases/download/1.27.3/istio-1.27.3-win.zip
    ```      
...
2.  Add the `istioctl` client to your executables.
    ...
    ```bash       
    export PATH=./istio-1.27.3/bin:$PATH
    ```        
...
3.  Install Istio into the cluster:
    ...
    ```output   
    > This will install the Istio 1.27.3 demo profile with ["Istio core" "Istiod" "Ingress gateways" "Egress gateways"] components into the cluster. Proceed? (y/N) y
...
6.  Run the following
    ...
    ```shell script
    kubectl apply -f ./istio-1.27.3/samples/addons/grafana.yaml
    kubectl apply -f ./istio-1.27.3/samples/addons/prometheus.yaml
    kubectl apply -f ./istio-1.27.3/samples/addons/jaeger.yaml
    kubectl apply -f ./istio-1.27.3/samples/addons/kiali.yaml
...
If you get Image Inspection errors, please follow this solution:
